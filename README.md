...
- `StonekeepRegistry.sol` — the core registry. Handles registering and looking up proof of authorship. Once something is registered here, it's permanent. Requires an explicit self-attestation from the registrant, and lets the original author flag their own registration as disputed if needed. Emits `WorkRegistered` and `WorkDisputed` events, so external platforms and future indexers can track activity in real time rather than scanning every block.
...
- StonekeepRegistry: `0xF93a927d9A7449aF5E3573646301EB0BF5Df5395`
- RightsAssignment: `0xb09942634F4d57daA42258B6777969E64e83f74c`

## Deployed on BOT Chain Mainnet

- StonekeepRegistry: `0xeCA06aDAD4e9D7469DDfd24c18D32b1d0CA38378`
- RightsAssignment: `0xfbA9529a6AC883479fA34f8a39CA9f31381b678a`
...
- `registerWork` requires explicit self-attestation of ownership; this doesn't prove ownership, it creates an on-record claim.
- `transferRights` restricts calls to the current rights holder only, verified either from prior transfer history or the original registry author. It also rejects transfers to the zero address and transfers to yourself.
